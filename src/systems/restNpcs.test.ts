...
describe('buildRestNpcs', () => {
  ...
  it('builds between the configured minimum and maximum', () => {
    for (const r of [0, 0.25, 0.5, 0.75, 0.99]) {
      const npcs = buildRestNpcs(spells, () => r)
  ...
  it('speaks the player’s own sample sentences', () => {
    const npcs = buildRestNpcs(spells, seq([0.9, 0.1, 0.4, 0.2, 0.7, 0.3]))
  ...
  it('carries the translation across with the sentence', () => {
    const npcs = buildRestNpcs([spells[0]], () => 0)
  ...
  it('only draws on words that actually have an example', () => {
    ...
    const npcs = buildRestNpcs(mixed, seq([0.8, 0.2, 0.6, 0.4]))
  ...
  it('still produces people when no word has an example', () => {
    ...
    const npcs = buildRestNpcs([spellWith('산', '')], () => 0.5)
...
describe('markSpoken', () => {
  const npcs = buildRestNpcs([spellWith('물', '물을 마셔요.')], () => 0.3)
