...
// Hashed over what is cached, not over what it is called.
//
// It used to hash the page and the list of names. Art filenames never change
// — cover.webp stays cover.webp — so replacing a picture left the cache name
// identical, an installed copy kept serving the old one out of its cache,
// and the only way to see the new art was to uninstall. Reading the bytes
// costs about a tenth of a second over the whole tree and is the difference
// between shipping art and appearing to.
const fingerprint = createHash('sha256').update(html)
for (const entry of precache) {
  fingerprint.update(entry)
  // './' is the index, whose bytes are already in `html` above.
  const file = join(DIST, entry.replace(/^\.\//, ''))
  if (entry !== './' && existsSync(file)) fingerprint.update(readFileSync(file))
}
